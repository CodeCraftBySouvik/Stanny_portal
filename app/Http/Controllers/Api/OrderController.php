...
class OrderController extends Controller
{
    ...
    // public function createOrder(Request $request, OrderRepository $orderRepo)
    // {
    //     // dd($request->all());
    //     DB::beginTransaction();
    ...
    public function createOrder(Request $request)
    {
        ...
        try {
            ...
            // Normalize Postman-style key/value payload
            if (isset($request[0]['key'])) {
                ...
                foreach ($request->all() as $row) {
                    ...
                    // Convert "items[0][remarks]=Urgent" into array
                    parse_str($row['key'] . '=' . $row['value'], $parsed);

                    $normalized = array_replace_recursive($normalized, $parsed);
                }
            ...
            $rules =[
                ...
                // --- Conditional for Collection = 1 ---
                'items.*.selectedCatalogue' => 'nullable|string',
                'items.*.page_number' => 'nullable|integer',
                'items.*.fitting' => 'nullable|string',
                'items.*.searchTerm' => 'nullable|string',
                // --- Media Uploads ---
                'imageUploads.*.*' => 'nullable|file|image|mimes:jpg,jpeg,png,webp|max:2048',
                'voiceUploads.*.*' => 'nullable|file|mimes:mp3,wav,ogg,m4a,wma,webm,mpga|max:5120',
                // --- Extra Measurement ---
                // 'extra_measurement' => 'nullable|array',
                // 'extra_measurement.*' => 'nullable|string',
                // 'extra_measurement.*' => 'nullable|in:mens_jacket_suit,ladies_jacket_suit,shirt,trouser',
                'bill_book_copy' => 'required|file|mimes:jpg,jpeg,png,webp,pdf|max:5120',
              ];
                ...
                if (!$hasCustomer) {
                    ...
                    $rules['customer_image'] = 'required|file|image|max:2048';
                }
            ...
            $user = $this->getAuthenticatedUser();
            $salesmanId = $user->id;
            $designation = (int) $user->designation;
            ...
            foreach ($validated['items'] as $index => $item) {
                ...
                // If item is actually being processed, enforce price
                if (($item['item_status'] ?? null) === 'Process') {

                        if ($price === null || $price <= 0) {
                            throw \Illuminate\Validation\ValidationException::withMessages([
                                "items.{$index}.price" => [
                                    "Price must be greater than 0 for processing items."
                                ],
                            ]);
                        }
                    }
                if ($selectedFabricId && $price !== null) {
                    // Find the fabric and check the threshold price
                    $fabricData = Fabric::find($selectedFabricId); 
            ...
            // User Create/Update
            $customer = User::find($validated['customer_id'] ?? null);
            if (!$customer) {
                ...
                $customer->update([
                    'prefix' => $validated['prefix'] ?? $customer->prefix,
                    'name' => $validated['name'] ?? $customer->name,
                    'business_type' => $validated['selectedBusinessType'] ?? $customer->business_type,
                    'company_name' => $request->company_name ?? $customer->company_name,
                    'employee_rank' => $validated['employee_rank'] ?? $customer->employee_rank,
                    'email' => $validated['email'] ?? $customer->email,
                    'dob' => $validated['dob'] ?? $customer->dob,
                    'country_id' => $request->country_id ?? $customer->country_id,
                    'country_code_phone' => $validated['phone_code'] ?? $customer->country_code_phone,
                    'phone' => $validated['phone'],
                    'country_code_alt_1' => $validated['alt_phone_code_1'] ?? $customer->country_code_alt_1,
                    'alternative_phone_number_1' => $validated['alt_phone_1'] ?? $customer->alternative_phone_number_1,
                    'country_code_alt_2' => $validated['alt_phone_code_2'] ?? $customer->country_code_alt_2,
                    'alternative_phone_number_2' => $validated['alt_phone_2'] ?? $customer->alternative_phone_number_2,
                    'user_type' => 1,
                ]);
            }
            ...
            // Billing Address
            UserAddress::updateOrCreate(
                ['user_id' => $customer->id, 'address_type' => 1],
                [
                    'address' => $validated['billing_address'] ?? $validated['landmark'] ?? '',
                    'city' => $validated['billing_city'] ?? $validated['city'] ?? '',
                    'state' => $validated['billing_state'] ?? $validated['state'] ?? '',
            ...
            foreach($whatsappMapping as $map){
                if(!empty($validated[$map['field']]) && !empty($validated[$map['flag']])){
                    ...
                    if(!$exists){
                        UserWhatsapp::updateOrCreate(
                            ['user_id'=>$customer->id,'whatsapp_number'=>$validated[$map['field']]],
                            ['country_code'=>$map['code']]
                        );
                    }
                }
            ...
            $billBookPath = $request->hasFile('bill_book_copy') 
                ? Helper::handleFileUpload($request->file('bill_book_copy'), 'bill_book_copy') 
                : null;
            ...
            $order = Order::create([
                ...
                'total_amount'=>$totalAmount,
                'paid_amount'=>$paidAmount,
                'physical_order_bill_book'=>$billBookPath,
                'last_payment_date'=>now(),
                'created_by'=> $salesmanId,
                'team_lead_id'=>$validated['team_lead_id'] ?? $user->parent_id ?? null,
            ]);
            ...
            // Order Items + Measurements + Extra fields + Images/Voice
            foreach ($validated['items'] as $k => $item) {
                if (($item['collection'] ?? null) == 1 && empty($item['page_item'])) {
                    $page = Page::where('catalogue_id', ($item['selectedCatalogue'] ?? null))
                        ->where('page_number', $item['page_number'])
                        ->first();
                ...
                $orderItem->remarks = $item['remarks'] ?? null;
                $orderItem->status = $item['item_status'] ?? 'Hold';
                $orderItem->piece_price = $item['price'] ?? 0;
                $orderItem->quantity = (($item['collection'] ?? null)) ? 1 : ($item['quantity'] ?? 1);
                $orderItem->fittings = (($item['collection'] ?? null) == 1) ? ($item['fitting'] ?? null) : null;

                $orderItem->priority_level = in_array($designation, [1, 4])
                    ? ($item['priority'] ?? null)
                    : null;
                ...
                if ($request->hasFile("imageUploads.$k")) {
                    $images = $request->file('imageUploads')[$k];
                    foreach ($images as $image) {
                            $path = $image->store('uploads/order_item_catalogue_images', 'public');
                            OrderItemCatalogueImage::create([
                ...
                // dd($request->voiceUploads[0]);
                if ($request->hasFile("voiceUploads.$k")) {
                    $voiceUploads = $request->file('voiceUploads');
                    if (isset($voiceUploads[$k]) && is_array($voiceUploads[$k])) {
                        foreach ($voiceUploads[$k] as $voice) {
                                $audioPath = $voice->store('uploads/order_item_voice_messages', 'public');
            ...
            // }
                    ...
                    $tlApproved = $order->items()
                        ->where('status', 'Process')
                        ->where('tl_status', 'Approved')
                        ->count();

                    $adminApproved = $order->items()
                        ->where('status', 'Process')
                    ...
                    if (in_array($designation, [1, 12])) {
                        // Admin / Store
                        $order->status = ($adminApproved === $totalItems)
                            ? 'Fully Approved By Admin'
                            : ($adminApproved > 0 ? 'Partial Approved By Admin' : 'Approval Pending');

                    } elseif ($designation === 4) {
                        // Team Lead
                        $order->status = ($tlApproved === $totalItems)
                            ? 'Fully Approved By TL'
                            : ($tlApproved > 0 ? 'Partial Approved By TL' : 'Approval Pending');
           ...
           //  Auto-approve only if logged-in user is Admin (1) or Store Person (12)
            // if ($user && in_array((int)$designation, [1, 12])) {
            //     $orderRepo->approveOrder($order->id, $user->id);
            // }
            if (in_array($designation, [1, 12]) && $adminApproved === $totalItems) {

                $airMail = $order->air_mail ?? 0;
                $order->update(['total_amount' => $order->total_product_amount + $airMail]);
                ...
                $packingSlip = PackingSlip::create([
                    ...
                    'created_by' => $salesmanId,
                    'created_at' => now(),
                    'disbursed_by' => $salesmanId,
                ]);

                // Create Invoice
                $lastInvoice = Invoice::latest()->first();
                $invoice_no = str_pad(optional($lastInvoice)->id + 1, 6, '0', STR_PAD_LEFT);
                // $invoiceNo = str_pad((Invoice::max('id') + 1), 6, '0', STR_PAD_LEFT);
